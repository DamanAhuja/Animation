...
namespace TensorFlowLite
{
        ...
        where T : struct
    {
        ...
        public BaseImagePredictor(byte[] modelData, InterpreterOptions options)
        {
            try
            {
                // Check if modelData is null or empty
                ...
                if (options == null)
                {
                    Debug.LogError("InterpreterOptions are null.");
                    return;
                }
            ...
            {
                Debug.LogError($"Error during interpreter initialization: {e.Message}");
                interpreter?.Dispose();
                throw;
            }
        ...
        public BaseImagePredictor(string modelPath, InterpreterOptions options)
            : this(LoadModelFromPath(modelPath), options)
        {
        }
        ...
        // Utility method to load model from StreamingAssets or file path
        private static byte[] LoadModelFromPath(string modelPath)
        {
            byte[] modelData = null;

            // Use the modelPath passed to the method to build the full path
            string fullPath = Path.Combine(Application.streamingAssetsPath, modelPath).Replace("\\", "/");

            Debug.Log("Checking model path: " + fullPath); // Debugging the full path
            if (File.Exists(fullPath))
            {
                modelData = File.ReadAllBytes(fullPath);
            }
            else
            {
